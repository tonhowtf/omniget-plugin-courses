...
struct SkoolDownloadCompleteEvent {
    group_name: String,
    success: bool,
    error: Option<String>,
}
...
pub async fn skool_login(
    ...
    *state.skool_courses_cache.lock().await = None;
    ...
    match api::authenticate(&email, &password).await {
        ...
        Err(e) => {
            tracing::error!("[skool] login failed: {}", e);
...
async fn fetch_skool_groups(
    ...
    let groups = api::list_groups(session)
        .await
...
pub async fn start_skool_course_download(
...

    {
        ...
        if map.contains_key(&group_id_hash) {
            return Err("Download already in progress for this group".to_string());
        }
        map.insert(group_id_hash, cancel_token.clone());
    }
    ...
    Ok(format!("Download started: {}", group_name))
}
