...
use crate::platforms::greennclub::api::{self, GreennCourse, GreennSession};
use crate::platforms::greennclub::downloader;
...
struct GreennDownloadCompleteEvent {
    course_name: String,
    success: bool,
    error: Option<String>,
}
...
#[tauri::command]
pub async fn greenn_login_token(
    state: tauri::State<'_, CoursesState>,
    token: String,
) -> Result<String, String> {
    let _ = api::delete_saved_session().await;
    state.greenn_session.lock().await.take();
    *state.greenn_session_validated_at.lock().await = None;
    *state.greenn_courses_cache.lock().await = None;

    let parsed_token = omniget_core::core::cookie_parser::parse_bearer_input(&token);

    let session = GreennSession {
        ...
        client: omniget_core::core::http_client::apply_global_proxy(reqwest::Client::builder())
            ...
            .default_headers({
                ...
                h.insert("Origin", "https://greenn.club".parse().unwrap());
                h.insert("Referer", "https://greenn.club/".parse().unwrap());
                h
            })
    ...
    match api::validate_token(&session).await {
        Ok(true) => {
            ...
            let mut guard = state.greenn_session.lock().await;
            *guard = Some(session);
            *state.greenn_session_validated_at.lock().await = Some(Instant::now());
...
#[tauri::command]
pub async fn greenn_check_session(
    state: tauri::State<'_, CoursesState>,
) -> Result<String, String> {
    let has_memory_session = state.greenn_session.lock().await.is_some();
    ...
    let guard = state.greenn_session.lock().await;
    let session = guard
        .as_ref()
        .ok_or_else(|| "not_authenticated".to_string())?;

    {
        let validated_at = state.greenn_session_validated_at.lock().await;
...
async fn fetch_greenn_courses(
    state: &tauri::State<'_, CoursesState>,
) -> Result<Vec<GreennCourse>, String> {
    let guard = state.greenn_session.lock().await;
    let session = guard
        .as_ref()
    ...
    let mut cache = state.greenn_courses_cache.lock().await;
    *cache = Some(crate::state::GreennCoursesCache {
...
#[tauri::command]
pub async fn greenn_list_courses(
    state: tauri::State<'_, CoursesState>,
) -> Result<Vec<GreennCourse>, String> {
    {
        let cache = state.greenn_courses_cache.lock().await;
    ...
    fetch_greenn_courses(&state).await
}

#[tauri::command]
pub async fn greenn_refresh_courses(
    state: tauri::State<'_, CoursesState>,
) -> Result<Vec<GreennCourse>, String> {
    {
        let mut cache = state.greenn_courses_cache.lock().await;
...
#[tauri::command]
pub async fn start_greenn_course_download(
    app: tauri::AppHandle,
    state: tauri::State<'_, CoursesState>,
    course_json: String,
    output_dir: String,
) -> Result<String, String> {
    let course: GreennCourse =
    ...
    tokio::spawn(async move {
        ...
        match result {
            ...
            Err(e) => {
                tracing::error!("[greenn] download error for '{}': {}", course.name, e);
                let _ = app.emit(
                    "download-complete",
                    &GreennDownloadCompleteEvent {
                        course_name: course.name,
