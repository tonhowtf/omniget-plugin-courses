...
use crate::platforms::kirvano::api::{self, KirvanoCourse, KirvanoSession};
use crate::platforms::kirvano::downloader;
...
struct KirvanoDownloadCompleteEvent {
    course_name: String,
    success: bool,
    error: Option<String>,
}
    ...
    *state.kirvano_courses_cache.lock().await = None;
    ...
    match api::authenticate(&email, &password).await {
        Ok(session) => {
            ...
            let mut guard = state.kirvano_session.lock().await;
            *guard = Some(session);
            *state.kirvano_session_validated_at.lock().await = Some(Instant::now());
            Ok(response_email)
        }
        Err(e) => {
            tracing::error!("[kirvano] login failed: {}", e);
...
#[tauri::command]
pub async fn kirvano_login_token(
    state: tauri::State<'_, CoursesState>,
    token: String,
) -> Result<String, String> {
    let _ = api::delete_saved_session().await;
...
#[tauri::command]
pub async fn kirvano_check_session(
    state: tauri::State<'_, CoursesState>,
) -> Result<String, String> {
    let has_memory_session = state.kirvano_session.lock().await.is_some();
    ...
    let guard = state.kirvano_session.lock().await;
    let session = guard
        .as_ref()
    ...
    {
        let validated_at = state.kirvano_session_validated_at.lock().await;
...
async fn fetch_kirvano_courses(
    state: &tauri::State<'_, CoursesState>,
) -> Result<Vec<KirvanoCourse>, String> {
    let guard = state.kirvano_session.lock().await;
    let session = guard
        .as_ref()
    ...
    let mut cache = state.kirvano_courses_cache.lock().await;
    *cache = Some(crate::state::KirvanoCoursesCache {
...
#[tauri::command]
pub async fn kirvano_list_courses(
    state: tauri::State<'_, CoursesState>,
) -> Result<Vec<KirvanoCourse>, String> {
    {
        let cache = state.kirvano_courses_cache.lock().await;
    ...
    fetch_kirvano_courses(&state).await
}

#[tauri::command]
pub async fn kirvano_refresh_courses(
    state: tauri::State<'_, CoursesState>,
) -> Result<Vec<KirvanoCourse>, String> {
    {
        let mut cache = state.kirvano_courses_cache.lock().await;
...
#[tauri::command]
pub async fn start_kirvano_course_download(
    app: tauri::AppHandle,
    state: tauri::State<'_, CoursesState>,
    course_json: String,
    output_dir: String,
) -> Result<String, String> {
    let course: KirvanoCourse =
