...
use crate::platforms::fluencyacademy::api::{self, FluencyCourse, FluencySession};
use crate::platforms::fluencyacademy::downloader;
...
struct FluencyDownloadCompleteEvent {
    course_name: String,
    success: bool,
    error: Option<String>,
}
...
#[tauri::command]
pub async fn fluency_login(
    state: tauri::State<'_, CoursesState>,
    email: String,
    password: String,
) -> Result<String, String> {
    let _ = api::delete_saved_session().await;
    state.fluency_session.lock().await.take();
    *state.fluency_session_validated_at.lock().await = None;
    *state.fluency_courses_cache.lock().await = None;

    match api::authenticate(&email, &password).await {
        Ok(session) => {
            let response_email = session.email.clone();
            let _ = api::save_session(&session).await;
            let mut guard = state.fluency_session.lock().await;
            *guard = Some(session);
            *state.fluency_session_validated_at.lock().await = Some(Instant::now());
            Ok(response_email)
        }
        Err(e) => {
            tracing::error!("[fluency] login failed: {}", e);
...
#[tauri::command]
pub async fn fluency_login_token(
    state: tauri::State<'_, CoursesState>,
    token: String,
) -> Result<String, String> {
    let _ = api::delete_saved_session().await;
    ...
    let session = FluencySession {
        ...
        client: omniget_core::core::http_client::apply_global_proxy(reqwest::Client::builder())
            ...
            .default_headers({
                ...
                h.insert("Origin", "https://academy.fluency.io".parse().unwrap());
                h.insert("Referer", "https://academy.fluency.io/".parse().unwrap());
                h
            })
...
#[tauri::command]
pub async fn fluency_check_session(
    state: tauri::State<'_, CoursesState>,
) -> Result<String, String> {
    let has_memory_session = state.fluency_session.lock().await.is_some();
    ...
    let guard = state.fluency_session.lock().await;
    let session = guard
        .as_ref()
    ...
    {
        let validated_at = state.fluency_session_validated_at.lock().await;
...
async fn fetch_fluency_courses(
    state: &tauri::State<'_, CoursesState>,
) -> Result<Vec<FluencyCourse>, String> {
    let guard = state.fluency_session.lock().await;
    let session = guard
        .as_ref()
    ...
    let mut cache = state.fluency_courses_cache.lock().await;
    *cache = Some(crate::state::FluencyCoursesCache {
...
#[tauri::command]
pub async fn fluency_list_courses(
    state: tauri::State<'_, CoursesState>,
) -> Result<Vec<FluencyCourse>, String> {
    {
        let cache = state.fluency_courses_cache.lock().await;
    ...
    fetch_fluency_courses(&state).await
}

#[tauri::command]
pub async fn fluency_refresh_courses(
    state: tauri::State<'_, CoursesState>,
) -> Result<Vec<FluencyCourse>, String> {
    {
        let mut cache = state.fluency_courses_cache.lock().await;
...
#[tauri::command]
pub async fn start_fluency_course_download(
    app: tauri::AppHandle,
    state: tauri::State<'_, CoursesState>,
    course_json: String,
    output_dir: String,
) -> Result<String, String> {
    let course: FluencyCourse =
    ...
    tokio::spawn(async move {
        ...
        match result {
            ...
            Err(e) => {
                tracing::error!("[fluency] download error for '{}': {}", course.name, e);
                let _ = app.emit(
                    "download-complete",
                    &FluencyDownloadCompleteEvent {
                        course_name: course.name,
