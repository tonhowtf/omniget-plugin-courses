...
pub async fn wondrium_login(
    ...
    plugin.wondrium_session.lock().await.take();
    *plugin.wondrium_session_validated_at.lock().await = None;
    *plugin.wondrium_courses_cache.lock().await = None;

    match api::authenticate(&email, &password).await {
        Ok(session) => {
            let response_email = session.email.clone();
            let _ = api::save_session(&session).await;
            let mut guard = plugin.wondrium_session.lock().await;
            *guard = Some(session);
            *plugin.wondrium_session_validated_at.lock().await = Some(Instant::now());
            Ok(response_email)
        }
        Err(e) => {
...
pub async fn wondrium_check_session(
    ...
    let has_memory_session = plugin.wondrium_session.lock().await.is_some();
    ...
    let guard = plugin.wondrium_session.lock().await;
    let session = guard
        .as_ref()
    ...
    {
        let validated_at = plugin.wondrium_session_validated_at.lock().await;
...
async fn fetch_wondrium_courses(
    ...
    let mut cache = plugin.wondrium_courses_cache.lock().await;
...
pub async fn wondrium_list_courses(
    ...
    {
        let cache = plugin.wondrium_courses_cache.lock().await;
    ...
    fetch_wondrium_courses(&plugin).await
}
        ...
        match result {
            Ok(()) => {
                let _ = host.emit_event(
                    "download-complete", serde_json::to_value(&WondriumDownloadCompleteEvent {
