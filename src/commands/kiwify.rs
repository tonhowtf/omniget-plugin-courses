...
pub async fn kiwify_login(
    ...
    *plugin.kiwify_courses_cache.lock().await = None;

    match api::authenticate(&email, &password).await {
        Ok(session) => {
            let response_email = session.email.clone();
            let _ = api::save_session(&session).await;
            let mut guard = plugin.kiwify_session.lock().await;
            *guard = Some(session);
            *plugin.kiwify_session_validated_at.lock().await = Some(Instant::now());
            Ok(response_email)
        }
        Err(e) => {
...
pub async fn kiwify_check_session(
    ...
    {
        let validated_at = plugin.kiwify_session_validated_at.lock().await;
...
async fn fetch_kiwify_courses(
    ...
    let mut cache = plugin.kiwify_courses_cache.lock().await;
    *cache = Some(crate::state::KiwifyCoursesCache {
...
pub async fn kiwify_list_courses(
    ...
    {
        let cache = plugin.kiwify_courses_cache.lock().await;
