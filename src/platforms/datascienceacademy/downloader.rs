...
use super::api::{self, DsaCourse, DsaSession};

#[derive(Debug, Clone, serde::Serialize)]
pub struct DsaCourseDownloadProgress {
...
pub async fn download_full_course(
    ...
    if omniget_core::core::course_utils::is_course_complete(&course_dir) {
        tracing::info!("[dsa] course already complete, skipping");
        return Ok(());
    }
                ...
                if let Some(ref vimeo_id) = lesson.vimeo_id {
                    ...
                    if tokio::fs::try_exists(&video_path).await.unwrap_or(false) {
                        ...
                        if meta.map(|m| m.len() > 0).unwrap_or(false) {
                            tracing::info!("[dsa] Skipping existing: {}", video_path);
                            let done = completed.fetch_add(1, Ordering::Relaxed) + 1;
                            let _ = app.emit(
                                "download-progress",
                                &DsaCourseDownloadProgress {
                    ...
                    match download_with_ytdlp(&vimeo_url, &mod_dir, &cancel_token).await {
                        ...
                        Err(e) => {
                            tracing::error!(
                                "[dsa] yt-dlp download failed for '{}': {}",
                                lesson.name,
                                e
                            );
                        }
...
async fn download_with_ytdlp(
    ...
    let result = omniget_core::core::ytdlp::download_video(
        ...
        None,
        None,
        Some("https://www.datascienceacademy.com.br/"),
        cancel_token.clone(),
        None,
        8,
        false,
