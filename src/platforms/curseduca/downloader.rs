...
use super::api::{self, CurseducaCourse, CurseducaSession};

#[derive(Debug, Clone, serde::Serialize)]
pub struct CurseducaDownloadProgress {
...
pub async fn download_full_course(
    ...
    if omniget_core::core::course_utils::is_course_complete(&course_dir) {
        tracing::info!("[curseduca] course already complete, skipping");
        return Ok(());
    }
    ...
    for (mi, module) in modules.iter().enumerate() {
        ...
        for (li, lesson) in module.lessons.iter().enumerate() {
            ...
            let lesson_uuid = lesson.uuid.as_deref().unwrap_or(&lesson.id);

            let video_url = match api::get_lesson_video_url(session, lesson_uuid).await {
                Ok(Some(url)) => url,
                Ok(None) => {
                    tracing::warn!("[curseduca] No video found for lesson '{}'", lesson.name);
                ...
                Err(e) => {
                    tracing::error!("[curseduca] Failed to get video URL for '{}': {}", lesson.name, e);
                    let done = completed.fetch_add(1, Ordering::Relaxed) + 1;
                    let _ = app.emit(
                        "download-progress",
                        &CurseducaDownloadProgress {
            ...
            if tokio::fs::try_exists(&video_path).await.unwrap_or(false) {
                ...
                if meta.map(|m| m.len() > 0).unwrap_or(false) {
                    tracing::info!("[curseduca] Skipping existing: {}", video_path);
                    let done = completed.fetch_add(1, Ordering::Relaxed) + 1;
                    let _ = app.emit(
                        "download-progress",
                        &CurseducaDownloadProgress {
            ...
            if video_url.contains(".m3u8") {
                match download_hls_video(&video_url, &video_path, &cancel_token).await {
                    ...
                    Err(e) => {
                        tracing::error!("[curseduca] HLS download failed for '{}': {}", lesson.name, e);
                ...
                match download_with_ytdlp(&video_url, &mod_dir, &cancel_token).await {
                    ...
                    Err(e) => {
                        tracing::error!("[curseduca] yt-dlp download failed for '{}': {}", lesson.name, e);
                    }
                }
            }
