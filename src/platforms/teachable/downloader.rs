...
use super::api::{self, TeachableCourse, TeachableSession};

#[derive(Debug, Clone, serde::Serialize)]
pub struct TeachableCourseDownloadProgress {
...
pub async fn download_full_course(
    ...
    let modules = api::get_syllabus(session, &course.school_id, &course.id).await?;
    ...
    let _ = host.emit_event(
        "download-progress", serde_json::to_value(&TeachableCourseDownloadProgress {
    ...
    for (mi, module) in modules.iter().enumerate() {
        ...
        for (li, lesson) in module.lessons.iter().enumerate() {
            ...
            let detail = match api::get_lecture_detail(session, &course.school_id, &course.id, &lesson.id).await {
                ...
                Err(e) => {
                    tracing::error!(
                        "[teachable] Failed to get lecture detail '{}': {}. Continuing...",
                        lesson.name,
                        e
                    );
            ...
            if let Some(ref video_url) = detail.video_url {
                ...
                if video_url.contains(".m3u8") {
                    let referer = detail.video_referer.as_deref().unwrap_or("https://teachable.com/");
                    match download_hls_video(video_url, &video_path, referer, &cancel_token).await {
                        ...
                        Err(e) => {
                            tracing::error!(
                                "[teachable] HLS download failed for '{}': {}",
                                lesson.name,
                                e
                            );
                    ...
                    match download_with_ytdlp(video_url, &lesson_dir, referer, &cancel_token).await {
                        ...
                        Err(e) => {
                            tracing::error!(
                                "[teachable] yt-dlp download failed for '{}': {}, trying direct download",
                                lesson.name,
                                e
                            );
                            match download_file_direct(&session.client, video_url, &video_path, &cancel_token).await {
                                ...
                                Err(e2) => {
                                    tracing::error!(
                                        "[teachable] Direct download also failed for '{}': {}",
                                        lesson.name,
                                        e2
                                    );
                                }
            ...
            for file in &detail.files {
                ...
                if let Some(ref url) = file.url {
                    match download_file_direct(&session.client, url, &file_path, &cancel_token).await {
                        ...
                        Err(e) => {
                            tracing::error!(
                                "[teachable] File download failed for '{}': {}",
                                file.name,
                                e
                            );
                        }
