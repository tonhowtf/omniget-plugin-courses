...
impl AfyaInternatoDownloader {
    pub fn new() -> Self {
        Self
    }
}

impl Default for AfyaInternatoDownloader {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl PlatformDownloader for AfyaInternatoDownloader {
    fn name(&self) -> &str {
        "afyainternato"
    }
    ...
    fn can_handle(&self, url: &str) -> bool {
        if let Ok(parsed) = url::Url::parse(url) {
            if let Some(host) = parsed.host_str() {
                ...
                return host == "afya.com.br"
                    || host.ends_with(".afya.com.br");
            }
        }
        false
    }
